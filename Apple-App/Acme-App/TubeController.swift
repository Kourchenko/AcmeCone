...
class TubeController: UIViewController, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
        ...
        if ((!quantity.isEmpty)
            ...
            if (_optional.isEmpty) {
                if ((Int(quantity) == nil)
                    ...
                    let results = TUBES.filter {$0.id == id}
                    if (results.isEmpty) {
                        TUBES.append(tube)
                        self.dismissViewControllerAnimated(true, completion: nil)
                    } else {
                        let alert: UIAlertView = UIAlertView(title: "", message: "These Tube Wrap Measurements exist in your Cart!", delegate: nil, cancelButtonTitle: "OK");
